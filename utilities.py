...
def compute_sum_capacity(features_df, time_series):
    # exclude N_ph and g_2_correlator
    features_df = features_df.drop(['N_ph', 'g_2_correlator'], axis=1)
    """
    # take each multiple space to a single space for each element of each column
    for col in features_df.columns:
        features_df[col] = features_df[col].replace(
            ' +', ',', regex=True)
        for i in range(len(features_df[col])):
            features_df.loc[i, col] = features_df.loc[i, col].replace('[,', '[').replace(',]', ']').replace(',,', ',')
        features_df[col] = features_df[col].apply(ast.literal_eval)
    """
    tot_width = 0
    for col in features_df.columns:
        # select first element
        first_element = features_df[col][0]
        # compute len
        len_element = len(first_element)
        # sum
        tot_width += len_element

    # ML part
    N_raw_data = len(features_df)
    # fraction_skip = 0.1
    # N_skip = int(fraction_skip*N_raw_data)
    N_skip = 20
    ...
    for k in range(N_skip, data_size):
        Z_exp_val = pt.tensor(features_df['Z'][k], dtype=float)
        Y_exp_val = pt.tensor(features_df['Y'][k], dtype=float)
        X_exp_val = pt.tensor(features_df['X'][k], dtype=float)

        XX_exp_val = pt.tensor(features_df['XX'][k], dtype=float)
        YY_exp_val = pt.tensor(features_df['YY'][k], dtype=float)
        ZZ_exp_val = pt.tensor(features_df['ZZ'][k], dtype=float)

        X_data[k] = pt.cat((X_exp_val, Y_exp_val, Z_exp_val,
                        XX_exp_val, YY_exp_val, ZZ_exp_val))
        Y_ini_data[k] = time_series[k]

    tau_vec = np.arange(1, 21)
    capacities_train = []
    capacities_test = []
    for tau in tau_vec:
        # -tau - gives future prediction
        # +tau - gives memory
        Y_data = pt.roll(Y_ini_data, -tau)
        ...
        N_test = int(fraction_test*N_total)
    ...
    # sum capacity train
    sum_capacity_train = np.sum(capacities_train)
    
    return sum_capacity_test, sum_capacity_train

def generate_random_J_couplings(L, path, name_file):
    J_L = np.random.uniform(low = -0.5, high = 0.5, size=(L,L))
    J_L = np.triu(J_L, k = 1)
    J_L = 0.5*(J_L  + J_L.T)

    for i in range(L):
        J_L[i,i] = 0 # exclude spin self-interaction
 
    filename_J = path + name_file + ".txt"
    np.savetxt(filename_J, J_L)